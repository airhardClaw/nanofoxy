...
def gateway(
    ...
    async def on_heartbeat_documente(tasks: str, response: str) -> None:
        """Document heartbeat tasks and results to memory files."""
        ...
        # Write tasks file (mittel: task + kurzes resultat)
        tasks_file = tasks_dir / f"{date_str}.md"
        tasks_content = f"""# Heartbeat Tasks - {date_str}
...
"""
    ...
    heartbeat = HeartbeatService(
        ...
        on_documente=on_heartbeat_documente,
...
def status():
    ...
    if config_path.exists():
        ...
        if search_config.api_key:
            console.print("  Search API key: [green]✓[/green]")
        else:
            console.print("  Search API key: [dim]not set (using DuckDuckGo fallback)[/dim]")
...
def memory(
    ...
    if status:
        from nanobot.agent.memory import PromotionTracker, ShortTermRecallStore
